...
export const metadata: Metadata = buildMetadata({
  title: "Buy AI Subscriptions in Dhaka — bKash/Nagad Payment",
  description: "Buy ChatGPT Plus, Claude Pro, Midjourney & 103+ AI tools in Dhaka, Bangladesh. bKash/Nagad payment. 5-15 min WhatsApp delivery. a growing community of customers.",
  canonical: "https://aipremiumshop.com/dhaka",
});
...
export default function DhakaPage() {
  return (<>
    ...
    <BreadcrumbJsonLd items={[{ name: "Home", path: "/" }, { name: "Dhaka", path: "/dhaka" }]} />
    <main className="min-h-screen"><section className="mx-auto max-w-4xl px-5 sm:px-8 py-12">
      <p className="text-[0.6875rem] font-semibold uppercase tracking-[0.15em] text-[#f4b942]"><MapPin className="size-3 inline mr-1" />Dhaka</p>
      <h1 className="mt-3 text-4xl sm:text-5xl font-bold tracking-tight">Premium AI Subscriptions in Dhaka</h1>
      <p className="mt-4 text-lg leading-7 text-[#8a91a8]">Buy ChatGPT, Claude, Midjourney & 103+ AI tools from Dhaka with bKash/Nagad. 5-15 min WhatsApp delivery.</p>
      ...
      <div className="mt-6 p-4 rounded-xl border border-[#f4b942]/10 bg-[#f4b942]/[0.03]">
        ...
        <p className="mt-2 text-[0.875rem] leading-relaxed text-[#c9ceda]">
          Yes — you can buy <strong>103+ premium AI subscriptions from Dhaka</strong> through AI Premium Shop. Pay with bKash, Nagad, or Rocket — no international card needed. Delivery in 5-15 minutes via WhatsApp. Trusted by a growing community of customers across all 64 districts since 2024.
        </p>
      </div>
      ...
      <div className="mt-6 p-4 rounded-xl border border-white/[0.04] bg-white/[0.02]">
        ...
        <p className="text-[0.875rem] leading-relaxed text-[#8a91a8]">
          <strong>Dhaka থেকে bKash/Nagad দিয়ে AI সাবস্ক্রিপশন কিনুন —</strong> ChatGPT Plus (৳৩৫০), Google AI Pro (৳৫০০), Midjourney (৳১,১৯৯) সহ ১০৩+ টুল। WhatsApp-এ ৫-১৫ মিনিটে ডেলিভারি। ২০২৪ সাল থেকে ৩,০০০+ কাস্টমার।
        </p>
      </div>
      ...
      <div className="mt-10 flex justify-center">
        <a href={`https://wa.me/${process.env.NEXT_PUBLIC_WA_PRIMARY ?? "8801865385348"}?text=${encodeURIComponent("Hi, I am from Dhaka. I want an AI subscription.")}`} className="btn-whatsapp h-12 px-8 text-[0.875rem]">
          <MessageCircle className="size-4" /> Order from Dhaka <ArrowRight className="size-4" />
        </a>
      </div>
